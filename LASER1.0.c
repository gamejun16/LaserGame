...
void MENUAL(void)
{
	...
	gotoxy(2, 11); printf("레이저가 발사되는 간격과 갯수가 줄어듭니다.");
	...
	gotoxy(2, 20); printf("레이저가 발사되는 간격을 보여줍니다.");
	...
	gotoxy(2, 8); printf("현재 버전 : 1.01");
	gotoxy(2, 10); printf("【 패치 노트 】");
	gotoxy(2, 11); printf("각 stage의 레이저 최대 발사 갯수를 제한시켜");
	gotoxy(2, 12); printf("stage의 의미를 명확히 함.");
	gotoxy(2, 13); printf("레이저가 발사될 곳을 더 명확히 알 수 있게 함.");
	gotoxy(2, 15); printf("【 수정해야 할 사항 】");
	gotoxy(2, 16); printf("간혹 죽은 뒤 ENTER를 여러 차례 입력해야 하는");
	gotoxy(2, 17); printf("오류.");
	gotoxy(2, 18); printf("MANUAL 두번째 페이지가 자동으로 넘어가는 오류.");

	Sleep(500); // 0.5초 짧은 딜레이
	/*menual_exit = _getch();
	if (menual_exit != '|') return;*/
	while (1) {
		if (clock() % 2000 == 1000) { gotoxy(4, 22); printf("▶▶ENTER를 누르십시오▶▶"); }
		else if (clock() % 2000 == 0) { gotoxy(4, 22); printf("　　      　　      　 　　　　　　　"); }
		if (_kbhit()) { getchar(); break; }
...
void field_data(void) // field()로 생성된 화면의 UI탭에 각종 수치를 채워 넣는 함수
{
	...
	gotoxy(27, 20); printf("%4d", ms_time);
	gotoxy(27, 21); printf("%4.1f", (ms_time / 1000.0));
}
...
void character(void)
{
	...
	if (_kbhit()) {
		...
		if (key == 72 && char_ypos != 4) char_ypos--;
		else if (key == 80 && char_ypos != 22) char_ypos++;
		else if (key == 75 && char_xpos != 4) char_xpos--;
		else if (key == 77 && char_xpos != 22) char_xpos++;
...
void smash_view(void)
{
		...
		if (exit_died_view == 13) { smash_check = 1;
		left_count = 10;
		score = 0; // 점수 초기화
		char_xpos = 13; char_ypos = 13; // Player 좌표 초기화
		difficulty = 0; // 난이도 초기화 (= 레이저의 개수와 관련)
